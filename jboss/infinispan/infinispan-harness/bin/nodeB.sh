...
while [ "$1" != "" ]; do
    if [ "$1" = "-debug" ]; then
        ...
        DEBUG_OPTS_SHMEM="-Xdebug -Xnoagent -Djava.compiler=NONE -Xrunjdwp:transport=dt_shmem,server=y,suspend=y,address=B"
...
${JAVA_HOME}/bin/java -Xmx4128m ${JAVA_OPTS} -cp "${CLASSPATH}" -Dplayground.infinispan.node.config.file=squaretrade-infinispan.xml -Dlog.file.name=${reldir}/../logs/nodeB.log -Djgroups.tcp.port=7801 com.novaordis.playground.infinispan.Main ${NODE_NAME}
